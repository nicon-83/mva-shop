...
class Product
{
	 ...
	 */
	public static function getLatestProducts($count = self::SHOW_BY_DEFAULT, $page)
	{

		//Приводим переменные к int
		$count = intval($count);
		$page = intval($page);

		//Задаем смещение для sql запроса(необходимо для пагинации)
		$offset = ($page -1) * $count;
	...
	/**
	 * Возвращает количество товаров в наличии на сайте
	 * @return integer <p>количество товаров в наличии на сайте</p>
	 */
	public static function getTotalProducts()
	{
		...
		// Получение и возврат результатов
		$result = $db->query('SELECT COUNT(*) AS count FROM `product` WHERE status="1"');
		$result->setFetchMode(PDO::FETCH_ASSOC);
		$row = $result->fetch();
		return intval($row['count']);
	}
